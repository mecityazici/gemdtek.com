                ...
                <a href="{{ route('news.show', $post) }}"
                   class="group bg-white rounded-xl overflow-hidden shadow-sm border border-graphite/5 hover:shadow-lg transition-all">
                    <div class="aspect-[16/9] bg-navy-100 relative overflow-hidden">
                        @if ($post->cover_url)
                            <img src="{{ $post->cover_thumb_url }}" srcset="{{ $post->cover_thumb_url }} 400w, {{ $post->cover_web_url }} 1280w" sizes="(max-width: 768px) 100vw, 33vw" alt="" loading="lazy" decoding="async" width="400" height="225" class="w-full h-full object-cover group-hover:scale-105 transition-transform">
                        @else
