...
@section('title', 'Başvurular — GEMDTEK')

@section('content')

<section class="bg-navy-900 text-cream">
    <div class="container-tight py-20 md:py-28">
        <p class="font-mono text-xs uppercase tracking-widest text-brass-300 mb-4">Başvurular</p>
        <h1 class="text-4xl md:text-5xl font-display font-bold mb-6 max-w-3xl leading-tight">
            Aramıza katılmak istiyorsan <span class="text-brass-400">doğru yerdesin.</span>
        </h1>
        <p class="text-lg text-cream/80 max-w-3xl">
            Aktif başvuru formlarımızı aşağıda bulabilirsin. Her form için tek bir başvuru hakkın var,
            başvurun sonrası ekibimiz e-posta ile dönüş yapacaktır.
        </p>
    </div>
</section>
...
<section class="container-tight py-20">
    @if ($forms->isEmpty())
        <div class="max-w-2xl mx-auto text-center bg-cream rounded-xl p-12 border border-graphite/10">
            <p class="font-display text-2xl text-navy-800 font-bold mb-3">Şu anda açık başvuru yok.</p>
            <p class="text-graphite/70">Dönem başvuruları açıldığında bu sayfada listelenecektir. Sosyal medya hesaplarımızı takipte kal.</p>
        </div>
    @else
            ...
            @foreach ($forms as $form)
                <a href="{{ route('forms.show', $form) }}"
                   class="group bg-white rounded-xl p-6 shadow-sm border border-graphite/5 hover:shadow-lg hover:border-brass-300 transition-all">
                    <p class="font-mono text-xs uppercase tracking-widest text-brass-600 mb-2">Açık başvuru</p>
                    ...
                    <div class="mt-4 flex items-center justify-between text-sm">
                        @if ($form->ends_at)
                            <span class="font-mono text-xs text-graphite/60">
                                Son tarih: {{ $form->ends_at->format('d M Y H:i') }}
                            </span>
                        @else
                            <span></span>
                        @endif
